 ...
 *
 *         Copyright (C) 2002 Michael Green <mtgreen@cs.ucsd.edu> Copyright (C)
 *         2002 Paul Kube <kube@cs.ucsd.edu> Copyright (C) 2005 Owen Astrachan
 *         <ola@cs.duke.edu>
 *
...
public class BoggleGUI extends JFrame {
	...
	public BoggleGUI(ILexicon lex, IWordOnBoardFinder finder,
			InputStream stream, IAutoPlayer compPlayer) {
	...
	private void setUpMenuBar() {
		...
		newRandom.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, Toolkit
		...
		gameTime.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String secs = JOptionPane.showInputDialog(BoggleGUI.this,
						"time in seconds");
				try {
			...
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(BoggleGUI.this,
						"Compsci Boggle, brought to you\n"
								...
								+ "including, of course, you.", "About Game",
	...
	private class BoggleBoardPanel extends JPanel {
		...
		private class DiePanel extends JPanel {
			...
			public void setFace(String chars) {
				if (chars.length() > 1) {
					face = chars.substring(0, 1)
							+ chars.substring(1).toLowerCase();
				} else {
					face = chars;
				}
			}
	...
	class PlayerView extends JPanel implements IPlayerView {
		...
		public PlayerView(String player) {
			...
			myWordList.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					java.util.List<BoardCell> list = myFinder.cellsForWord(
							myBoard, word);
		...
		public void showWord(String word,
				java.util.List<BoardCell> letterLocations, IPlayer player) {
			myWordList.add(word);
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (myLexicon.wordStatus(s) != LexStatus.WORD) {
						...
						if (!myString.toString().equals(s)) {
							...
							if (humanPlayer.add(textField.getText()
									.toLowerCase())) {
